...
trait EMR extends aws.AmazonElasticMapReduce {
  ...
  private def toCluster(summary: ClusterSummary): Cluster =
  ...
  def terminateCluster(jobFlowId: String) = terminateJobFlows(new TerminateJobFlowsRequest().withJobFlowIds(jobFlowId))
